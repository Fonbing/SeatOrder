...
//        canvas.translate(20,30);偏移位置，防止遮挡
        ...
        for (int i = 0; i < nodeProgressAdapter.getCount(); i++) {
            ...
            if (i == 0) {
                ...
                canvas.drawText(((RecordDetail) data.get(i)).getTime() + "", left * 2 + nodeRadius * 2 + 10, (i + 1) * nodeInterval + top - 20, mPaint);
//                canvas.drawText("不换行", left * 2 + nodeRadius * 2 + 10, i * nodeInterval + top + (nodeInterval / 3), mPaint);
                ...
                StaticLayout layout = new StaticLayout(((RecordDetail) data.get(i)).getContext() + "", textPaint, (int) (dWidth * 0.8), Layout.Alignment.ALIGN_NORMAL, 1.0F, 0.0F, true);
                canvas.save();
                ...
                canvas.drawText(((RecordDetail) data.get(i)).getTime() + "", left * 2 + nodeRadius * 2 + 10, (i + 1) * nodeInterval + top - 20, paint);
                //文字换行
                TextPaint textPaint = new TextPaint();
